...
async function findEpisodesExercises(client) {
  ...
  const ep2S2 = await bobRossCollection.findOne({ episode: "S02E02" });
  console.log(`The title of episode 2 in season 2 is ${ep2S2.title}`);

  // Find the season and episode number of the episode called "BLACK RIVER" [Should be: S02E06]
  const blackRiver = await bobRossCollection.findOne({ title: "BLACK RIVER" });
  console.log(
    `The season and episode number of the "BLACK RIVER" episode is ${blackRiver.episode}`
  );

  // Find all of the episode titles where Bob Ross painted a CLIFF [Should be: NIGHT LIGHT, EVENING SEASCAPE, SURF'S UP, CLIFFSIDE, BY THE SEA, DEEP WILDERNESS HOME, CRIMSON TIDE, GRACEFUL WATERFALL]
  const cliffEpisodesCursor = bobRossCollection.find({ elements: "CLIFF" });
  const cliffEpisodes = await cliffEpisodesCursor
    .map((doc) => doc.title)
    .toArray();
  console.log(
    `The episodes that Bob Ross painted a CLIFF are ${cliffEpisodes.join(", ")}`
  );

  // Find all of the episode titles where Bob Ross painted a CLIFF and a LIGHTHOUSE [Should be: NIGHT LIGHT]
  const cliffLighthouseCursor = bobRossCollection.find({
    elements: { $all: ["CLIFF", "LIGHTHOUSE"] },
  });
  const cliffLighthouseEpisodes = await cliffLighthouseCursor
    .map((doc) => doc.title)
    .toArray();
  console.log(
    `The episodes that Bob Ross painted a CLIFF and a LIGHTHOUSE are ${cliffLighthouseEpisodes.join(
      ", "
    )}`
  );
}
