...
async function q(db, label, sql, params = []) {
  ...
  console.log(`\n=== ${label} ===`);
  // print only the first 10 rows for readability
  console.table(res.rows.slice(0, 10));
  return res;
}

async function run() {
  const db = new Client(worldConfig);
  await db.connect();
  ...
  await q(
    ...
    `SELECT name FROM country WHERE population > 8000000 ORDER BY population DESC;`
  );
  ...
  await q(
    ...
    `SELECT name FROM country WHERE name ILIKE '%land%' ORDER BY name;`
  );
  ...
  await q(
    ...
    `SELECT c.population FROM city c
     JOIN country co ON c.countrycode = co.code
  ...
  await q(
    ...
    `SELECT SUM(population)::BIGINT AS world_population FROM country;`
  );
